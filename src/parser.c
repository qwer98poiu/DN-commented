...
convolutional_layer parse_convolutional(list *options, size_params params)
{
    ...
    convolutional_layer layer = make_convolutional_layer(batch,h,w,c,n,groups,size,stride,padding,activation, batch_normalize, binary, xnor, params.net->adam);//需要给权值和偏置分配内存，其它参数照抄
    //随机正态初始权值 rand_normal() at utils.c 从wiki扒的算法
    //根据pad和stride计算输出尺寸
    //给output和delta分配内存，元素个数为batch*outputs
    //如果有BN还要分配内存
...
route_layer parse_route(list *options, size_params params, network *net)
{
	...
	//n是层的个数，layers[i]和sizes[i]存放第i个层的层号和输出大小
    int *layers = calloc(n, sizeof(int));
    int *sizes = calloc(n, sizeof(int));
    for(i = 0; i < n; ++i){
        int index = parse_routeatoi(l);
        l = strchr(l, ',')+1;
        if(index < 0) index = params.index + index;
        layers[i] = index;
    ...
    //判断所有route源的输出尺寸是否相等，相等则把route层的c累加，不等则route层输出尺寸置0
    for(i = 1; i < n; ++i){
        int index = layers[i];
...
{
    list *sections = read_cfg(filename);//把cfg的内容存进列表
    node *n = sections->front;
    if(!n) error("Config file has no sections");
    network *net = make_network(sections->size - 1);//in network.c，根据section数分配空间，section数等于layer数
    net->gpu_index = gpu_index;//gpu_index是extern int
    //根据第一个section（必须是[net]）给param赋值
    size_params params;
    section *s = (section *)n->val;
    list *options = s->options;
    if(!is_network(s)) error("First section must be [net] or [network]");
    parse_net_options(options, net);//从第一个section的分链表options里找出各项参数，没有的赋默认值
    //注意net->batch /= subdiv *= time_steps
    ...
    while(n){
        ...
        //把中括号字符串（总链表中节点的type成员）转换成枚举型layer_type
        if(lt == CONVOLUTIONAL){
...
list *read_cfg(char *filename)
//返回一个链表，里面存放所有按[]划分的配置信息
{
    FILE *file = fopen(filename, "r");
    if(file == 0) file_error(filename);
    char *line;
    int nu = 0;
    list *options = make_list();//options是总链表，存放sections，每个中括号是一个section
    section *current = 0;
    while((line=fgetl(file)) != 0){
        ++ nu;
        strip(line);
            case '['://遇到中括号就新建一个section插进总链表
                current = malloc(sizeof(section));
                list_insert(options, current);
                current->options = make_list();//属于一个section的分链表，存一层的若干信息
                current->type = line;
                break;
            case '\0':
            case '#':
            case ';':
                free(line);
                break;
            default://中括号下的配置信息用read_option函数(in option_list.c)插入分链表
